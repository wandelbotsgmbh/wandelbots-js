...
function makeURLForWebSocket(path, user, password) {
  ...
  url2.protocol = url2.protocol.replace("https", "wss");
  if (user && password) {
    url2.username = user;
...
var AutoReconnectingWebsocket = class extends import_reconnecting_websocket.default {
  ...
  path;
  constructor(path, user, password) {
    super(() => makeURLForWebSocket(this.path, user, password), void 0, {
      startClosed: true
    });
    this.path = path;
...
var ConnectedMotionGroup = class _ConnectedMotionGroup {
  ...
  `,
    ...
    let isVirtual = false;
    try {
      const virtualModeRes = await nova.api.virtualRobotMode.getOperationMode(controllerId);
    ...
    const tcpOptionsRes = await nova.api.motionGroupInfos.listTcps(motionGroupId);
...
var NovaCellAPIClient = class {
  ...
  withCellId(ApiConstructor) {
    const apiClient = new ApiConstructor(this.config);
  ...
  deviceConfig = this.withCellId(import_wandelbots_api_client.DeviceConfigurationApi);
  motionGroup = this.withCellId(import_wandelbots_api_client.MotionGroupApi);
  motionGroupInfos = this.withCellId(import_wandelbots_api_client.MotionGroupInfosApi);
  controller = this.withCellId(import_wandelbots_api_client.ControllerApi);
  program = this.withCellId(import_wandelbots_api_client.ProgramApi);
  programValues = this.withCellId(import_wandelbots_api_client.ProgramValuesApi);
  controllerIOs = this.withCellId(import_wandelbots_api_client.ControllerIOsApi);
  motionGroupKinematic = this.withCellId(import_wandelbots_api_client.MotionGroupKinematicApi);
  motion = this.withCellId(import_wandelbots_api_client.MotionApi);
  coordinateSystems = this.withCellId(import_wandelbots_api_client.CoordinateSystemsApi);
  motionGroupJogging = this.withCellId(import_wandelbots_api_client.MotionGroupJoggingApi);
  virtualRobot = this.withCellId(import_wandelbots_api_client.VirtualRobotApi);
  virtualRobotSetup = this.withCellId(import_wandelbots_api_client.VirtualRobotSetupApi);
  virtualRobotMode = this.withCellId(import_wandelbots_api_client.VirtualRobotModeApi);
  virtualRobotBehavior = this.withCellId(import_wandelbots_api_client.VirtualRobotBehaviorApi);
  libraryProgramMetadata = this.withCellId(import_wandelbots_api_client.LibraryProgramMetadataApi);
  libraryProgram = this.withCellId(import_wandelbots_api_client.LibraryProgramApi);
  libraryRecipeMetadata = this.withCellId(import_wandelbots_api_client.LibraryRecipeMetadataApi);
  libraryRecipe = this.withCellId(import_wandelbots_api_client.LibraryRecipeApi);
};
...
var NovaClient = class {
  ...
  async connectMotionGroups(motionGroupIds) {
    ...
    return Promise.all(
      motionGroupIds.map(
        (motionGroupId) => ConnectedMotionGroup.connect(
          this,
          motionGroupId,
